...
const ChatWindow = ({
  ...
  const daysPlanned = getPlanDayCount(livePlan) || activeTrip.days;
  ...
  return (
    <section className="flex h-[100dvh] min-h-0 w-full max-w-full flex-col overflow-hidden rounded-none border border-slate-200/80 bg-white shadow-sm sm:h-[calc(100vh-1rem)] sm:rounded-xl lg:h-full lg:min-h-0">
      <header className="shrink-0 border-b border-slate-200 bg-white p-2.5">
        ...
        >
          <section className="rounded-md border border-slate-200 bg-[#f8faf9] px-2.5 py-2">
          ...
          {livePlan ? (
            <section className="rounded-md border border-slate-200 bg-[#f8faf9] px-2.5 py-2">
              <div className="flex items-center gap-3">
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-md bg-[#eef4ff] text-[#2563eb]">
                  <CalendarDays size={15} />
                </span>
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-lg font-semibold leading-tight text-slate-950">
                    {daysPlanned}
                  </p>
  ...
  return (
    <div ref={pickerRef} className="relative min-w-0 flex-1">
      ...
      <button
        ...
        className="mt-1 flex min-h-9 w-full items-center justify-between gap-3 rounded-md border border-slate-200 bg-white px-2.5 py-1 text-left transition hover:border-[#0f8f83] focus:border-[#0f8f83] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#bdeee6]"
      >
        <span>
          <span className="block text-[0.92rem] font-semibold leading-5 text-slate-950">
            {displayDate}
          </span>
          <span className="block text-[0.78rem] font-medium leading-4 text-slate-500">
            {helperText}
          </span>
        </span>
        <CalendarDays size={16} className="shrink-0 text-[#0f8f83]" />
      </button>
...
function getPlanDayCount(plan) {
  ...
  return Number(plan.days) || Number(plan.durationDays) || 0;
}
